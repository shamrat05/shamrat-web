...
import { Menu, X, Sun, Moon, Languages, Sparkles, Clapperboard, ChevronDown } from 'lucide-react';
      ...
      <div className="w-full max-w-5xl bg-neutral-900/80 backdrop-blur-md border border-white/10 rounded-full px-4 py-3 md:px-6 flex items-center justify-between shadow-2xl shadow-black/50 pointer-events-auto">
        ...
        >
          <div className="relative flex items-center justify-center">
            <Clapperboard className="w-5 h-5 text-white group-hover:text-neutral-300 transition-colors" />
          </div>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navItems.filter(item => ['about', 'projects', 'blog', 'contact'].includes(item.id)).map((item) => (
            <button
              key={item.id}
